...
const DEFAULT_SOURCE_CANDIDATES = [
  ...
  "F:\\个人知识库\\02-学习\\04-赵逍遥AI课\\潘AI的笔记.md"
];
...
function resolveSourcePath() {
  ...
  if (!found) {
    throw new Error(
      `未找到知识库路径。请设置 KB_SOURCE_PATH。尝试过: ${candidates.join(" | ")}`
    );
  }
  return found;
}
...
async function main() {
  ...
  for (const filePath of files) {
    ...
    fileCount += 1;
    const relPath = path.relative(sourcePath, filePath) || path.basename(filePath);
  ...
  console.log(`文件数: ${fileCount}, 分片数: ${chunks.length}`);
}
